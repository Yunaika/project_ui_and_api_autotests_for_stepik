...
def test_authorization_succeeded_by_registered_user(setup_browser):
    # PRECONDITION
...
@allure.title('Add course to wish list')
@pytest.mark.parametrize('window_size', ['desktop', 'tablet', 'mobile'], indirect=True)
def test_add_course_to_wishlist(setup_browser, course_id=95367):
    # PRECONDITION
...
def test_join_the_free_course_with_authorization(setup_browser, course_id=3356):
    # PRECONDITION
    with allure.step('Get course name'):
        course_name = stepik.get_course_name(course_id)
    ...
    with allure.step('Open active courses page'):
        # app.header_menu.click_on_logo()
        # app.learning_sidebar.click_on_item_courses_active()
...
@pytest.mark.parametrize('window_size', ['desktop'], indirect=True)
def test_join_the_free_course_without_authorization(setup_browser, course_id=3356):
    # WHEN
    with allure.step('Open course page'):
        app.given_opened_course_page(course_id)
...
@pytest.mark.parametrize('window_size', ['desktop'], indirect=True)
def test_search_free_course_with_certificate(setup_browser, search_query='Инди-курс программирования на Python'):
    # WHEN
...
def test_search_nonexistent_course(setup_browser, course_name='uberlebenskurs'):
    # WHEN
    app.given_opened_catalog()
...
@pytest.mark.parametrize('window_size', ['desktop'], indirect=True)
def test_open_news_popup_window_from_the_menu(setup_browser):
    # WHEN
    app.given_opened_main_page()
